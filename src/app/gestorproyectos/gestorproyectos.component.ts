...
@Component({
  selector: 'app-gestorproyectos',
  templateUrl: './gestorproyectos.component.html',
  styleUrls: ['./gestorproyectos.component.css']
})
export class GestorproyectosComponent implements OnInit {
  ...
  goToDatosProyecto(nProyecto: string, nUsuarioCreador: string) {
    ...
    this.router.navigate(['/datosproyecto']);
  }
  ...
  loadProjects(): void {
    this.authService.listarProyectos().subscribe(
      response => {
