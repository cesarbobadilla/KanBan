...
@Component({
  selector: 'app-usuariosasignar',
  templateUrl: './usuariosasignar.component.html',
  styleUrls: ['./usuariosasignar.component.css']
})
export class UsuariosasignarComponent implements OnInit{
  ...
  onEntregableSelect(nEntregable: number) {
    // Obtener nProyecto desde localStorage
    const nProyecto = localStorage.getItem('nProyecto');

    if (nProyecto) {
      // Guardar nEntregable en localStorage
      localStorage.setItem('nEntregable', nEntregable.toString());

      // Redirigir a /usuariolider
      this.router.navigate(['/usuariolider']);
    } else {
      console.error('nProyecto no encontrado en localStorage');
    }
  }
